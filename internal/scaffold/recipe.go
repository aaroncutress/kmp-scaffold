...
type Recipe struct {
	...
	// test source sets, a CI workflow, editor configuration. There is nothing
	// to call it, so no name is asked for and none is accepted, and applying it
	// twice is refused rather than producing a second copy.
	//
	// This is what makes an option chosen at generation time available
	// afterwards. A project generated with --no-tests is not a project that can
	// never have tests; it is one that does not have them yet.
	Singleton bool

	// AppliedTo reports whether this recipe has already been applied, for a
	// singleton whose answer lives somewhere the tool cannot see - a template's
	// own vars, usually. The manifest record is checked first, so this only has
	// to cover projects generated before the recipe existed.
	AppliedTo func(*model.Manifest) bool
...
// Applied reports whether a singleton recipe has already been applied.
//
// The manifest record is what a project generated by this build carries; a
// project generated before the recipe existed has no record, which is what
// AppliedTo is for - it can look at whatever the template does remember.
func (r Recipe) Applied(m *model.Manifest) bool {
	if !r.Singleton || m == nil {
		return false
	}
	if m.FindFeatureOf(r.Name, r.Name) != nil {
		return true
	}
	return r.AppliedTo != nil && r.AppliedTo(m)
}
